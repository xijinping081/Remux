...
#ifdef CONFIG_KSU_SUSFS_SUS_PATH
	if (nd->inode && unlikely(test_bit(AS_FLAGS_SUS_PATH, &nd->inode->i_state)) && likely(susfs_is_current_proc_umounted())) {
		return -ENOENT;
	}
#endif
...
#ifdef CONFIG_KSU_SUSFS_SUS_PATH
	if (link->dentry->d_inode && unlikely(test_bit(AS_FLAGS_SUS_PATH, &link->dentry->d_inode->i_state)) && likely(susfs_is_current_proc_umounted())) {
		return -ENOENT;
	}
#endif
...
{
#ifdef CONFIG_KSU_SUSFS_SUS_PATH
	if (unlikely(test_bit(AS_FLAGS_SUS_PATH, &inode->i_state)) && likely(susfs_is_current_proc_umounted())) {
		return -ENOENT;
	}
#endif
			   ...
			   struct path *path, struct path *root)
{
	...
	restore_nameidata();
#ifdef CONFIG_KSU_SUSFS_SUS_PATH
	if (!retval && path->dentry->d_inode && unlikely(test_bit(AS_FLAGS_SUS_PATH, &path->dentry->d_inode->i_state)) && likely(susfs_is_current_proc_umounted())) {
		putname(name);
		return -ENOENT;
	}
#endif
...
#ifdef CONFIG_KSU_SUSFS_SUS_PATH
	if (child->d_inode && unlikely(test_bit(AS_FLAGS_SUS_PATH, &child->d_inode->i_state)) && likely(susfs_is_current_proc_umounted())) {
		error = inode_permission2(mnt, dir, MAY_WRITE | MAY_EXEC);
		if (error) {
			return error;
		}
...
static int may_o_create(const struct path *dir, struct dentry *dentry, umode_t mode)
{
	...
	if (dentry->d_inode && unlikely(test_bit(AS_FLAGS_SUS_PATH, &dentry->d_inode->i_state)) && likely(susfs_is_current_proc_umounted())) {
...
skip_orig_flow:
...
#ifdef CONFIG_KSU_SUSFS_SUS_PATH
		if (unlikely(test_bit(AS_FLAGS_SUS_PATH, &dentry->d_inode->i_state)) && likely(susfs_is_current_proc_umounted())) {
			dput(dentry);
			return -ENOENT;
		}
#endif
	...
	if (dir_inode->i_op->atomic_open) {
		...
		if (unlikely(error == -ENOENT) && create_error)
			error = create_error;
#ifdef CONFIG_KSU_SUSFS_SUS_PATH
		if (!IS_ERR(dentry) && dentry->d_inode && unlikely(test_bit(AS_FLAGS_SUS_PATH, &dentry->d_inode->i_state)) && likely(susfs_is_current_proc_umounted())) {
			if (create_error) {
				dput(dentry);
				return create_error;
			}
